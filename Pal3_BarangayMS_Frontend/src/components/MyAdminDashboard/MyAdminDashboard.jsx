...
class MyAdminDashboard extends Component {
  ...
  toggleExtraPanels = () => {
    this.setState(prevState => ({
      showExtraPanels: !prevState.showExtraPanels
    }));
  };
    ...
    return (
      <Fragment>
        ...
        <div className='cards_ctr'>
          ...
          <button
            type="button"
            className="dashboard_toggle_btn"
            onClick={this.toggleExtraPanels}
          >
            <i className={`bi ${this.state.showExtraPanels ? 'bi-chevron-up' : 'bi-chevron-down'}`}></i>
            {' '}
            {this.state.showExtraPanels ? 'Hide Additional Dashboard Information' : 'Show Additional Dashboard Information'}
          </button>

          {this.state.showExtraPanels && (
            <div className='grid_ctr'>
              <div>
                <DashboardLogsCard
                  header={'Recent System Logs'}
                  data={DashboardStore.data.logsList}
                />
              </div>

              <div>
                <DashboardConnectionPanelCard
                  header={'Connection Details'}
                />

                <DashboardEmergencyHotline
                  header={"Emergency Hotlines"}
                  data={DashboardStore.data.hotlineList}
                />
              </div>
            </div>
          )}
